...
function Card({ children, className = "" }) {
  return (
    <div className={`bg-gradient-to-br from-slate-900 to-slate-800/80 border border-slate-700 rounded-2xl p-4 md:p-6 ${className}`}>
      {children}
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
        ...
        <span className="text-slate-400 text-sm">
          Analysis complete for <strong className="text-white break-words">{filename}</strong>
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <div className="inline-block px-5 py-1.5 rounded-full font-black text-xl mb-4 border break-words"
               style={{
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <div className="mb-5 flex flex-wrap">{data.skillsFound.map((s, i) => <Tag key={i} text={s} color="#22d3a5" />)}</div>
          <Section icon="💪" title="Strengths" />
          {data.strengths.map((s, i) => (
            <div key={i} className="flex gap-2 mb-2 items-start">
              <span className="text-emerald-400 mt-0.5 text-xs flex-shrink-0">▸</span>
              <span className="text-slate-400 text-sm leading-relaxed break-words">{s}</span>
            </div>
          ))}
        </Card>
        ...
        <Card>
          ...
          <div className="mb-2 flex flex-wrap">{data.jobRoles.map((r, i) => <Tag key={i} text={r} color="#a78bfa" />)}</div>
        </Card>
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {data.skillGaps.map((gap, i) => (
            <div key={i} className="bg-slate-950 rounded-xl p-4 border break-words"
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {data.improvements.map((imp, i) => (
            <div key={i} className="bg-slate-950 rounded-xl p-4 border-l-4 break-words"
